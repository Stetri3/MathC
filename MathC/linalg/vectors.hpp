...
struct Vec {
    ...
    //Uniforme. Impostiamo explicit per distinguere da impl conv (utile per strutture algebriche)
    explicit constexpr Vec(co::pass_t<T> init) {
        raw.fill(init);
    }
    ...
    //FUNZIONI ESPLICITE PER VARI PER

    // Prodotto esplicito scalare (Vettore * Vettore )
    template <typename U>
        requires co::ProdImageSemigroup<T, U>
    constexpr auto VecProd(const Vec<dim, U>& other) const {
    ...
    // Moltiplicazione esplicita per scalare (generico o di campo) (Ordine standard: Destrorso)
    template <typename U>
        requires requires(T t, U u) { t* u; }
    constexpr auto Scalar(const U& scalar) const {
        using ResultType = decltype(std::declval<T>()* std::declval<U>());
        Vec<dim, ResultType> result;
        for (size_t i = 0; i < dim; ++i) {
    ...
    // Scalare * Vettore (Sinistrorso)
    template <typename U>
        requires requires(U u, T t) { u* t; }
    constexpr auto ScalarLeft(const U& scalar) const {
        using ResultType = decltype(std::declval<U>()* std::declval<T>());
        Vec<dim, ResultType> result;
        for (size_t i = 0; i < dim; ++i) {
            result.raw[i] = scalar * raw[i];
        }
        return result;
    }
...
struct Mat {
    ...
    // Prodotto Matrice * Vettore (a destra)
    template <typename U>
        requires co::ProdImageSemigroup<T, U>
    constexpr auto VecProd(const Vec<Cols, U>& vec) const {
        using ProductType = decltype(std::declval<T>()* std::declval<U>());
    ...
    //Prodotto vettore * matrice (a sinistra)
    template <typename U>
        requires co::ProdImageSemigroup<U, T>
    constexpr auto VecProdLeft(const Vec<Rows, U>& vec) const {
        using ProductType = decltype(std::declval<U>()* std::declval<T>());
        Vec<Cols, ProductType> result;
        const U& v_val_0 = vec.raw[0];
        for (uint32_t c = 0; c < Cols; ++c) {
            result.raw[c] = v_val_0 * raw[c];
        }
        for (uint32_t r = 1; r < Rows; ++r) {
            const U& v_val = vec.raw[r];
            for (uint32_t c = 0; c < Cols; ++c) {
                result.raw[c] = result.raw[c] + (v_val * raw[r * Cols + c]);
            }
        }
        return result;
    }

    // Moltiplicazione Matrice * Scalare (a destra)
    template <typename U>
        requires requires(T t, U u) { t* u; }
    constexpr auto Scalar(const U& scalar) const {
        using ResultType = decltype(std::declval<T>()* std::declval<U>());
    ...
    template <uint32_t NewCols, typename U>
        requires co::ProdImageSemigroup<T, U> //Ripetiamo anche se è ereditato da Prod(), male non fa
    constexpr auto operator*(const Mat<Cols, NewCols, U>& other) const {
        return this->Prod(other);
    }

    // Overload Matrice * Vettore
    template <typename U>
        requires co::ProdImageSemigroup<T, U>
    constexpr auto operator*(const Vec<Cols, U>& vec) const {
        return this->VecProd(vec);
    }

    // Overload Vettore * Matrice 
    template <typename U>
        requires co::ProdImageSemigroup<U, T>
    friend constexpr auto operator*(const Vec<Rows, U>& vec, const Mat& mat) {
        return mat.VecProdLeft(vec);
    }

    // Overload Matrice * Scalare
    template <typename U>
        requires (!co::is_algebraic_v<U>&& requires(T t, U u) { t* u; })
    constexpr auto operator*(const U& scalar) const {
        return this->Scalar(scalar);
    }

    // Overload Scalare * Matrice
    template <typename U>
        requires (!co::is_algebraic_v<U>&& requires(T t, U u) { u* t; })
    friend constexpr auto operator*(const U& scalar, const Mat& mat) {
        return mat.ScalarLeft(scalar);
    }

    //Implicit access, Mat(row,col)
